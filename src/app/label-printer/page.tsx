...
export default function LabelPrinterPage() {
  ...
  const handlePrintAll = async () => {
    ...
    const labelsToPreview: BarcodeLabel[] = [];
    selectedItems.forEach(sel => {
      for (let i = 0; i < sel.copies; i++) {
        labelsToPreview.push({
          barcode: sel.item.barcode,
          productName: sel.item.name,
          price: sel.item.sellingPrice.toString(),
        });
    ...
    const previewStrips: BarcodeLabel[][] = [];
    for (let i = 0; i < labelsToPreview.length; i += 2) {
      previewStrips.push(labelsToPreview.slice(i, i + 2));
    }
              ...
              <Card className="bg-slate-900/50 border-slate-800">
                <CardHeader>
                  <CardTitle className="text-white flex items-center gap-2">
                    ...
                    Print Preview (Visual Layout)
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    {previewStrips.map((strip, idx) => (
                      <div key={idx} className="flex gap-2 p-2 bg-slate-950 border border-slate-800 rounded-md">
                        {strip.map((label, lIdx) => (
                          <div key={lIdx} className="flex-1 bg-white p-2 rounded border border-slate-200 text-black min-h-[120px] flex flex-col items-center justify-between text-center">
                            <div className="text-[10px] font-bold uppercase truncate w-full">SONAKSHI BOUTIQUE</div>
                            <div className="w-full flex flex-col items-center py-1">
                              <div className="h-6 w-full bg-slate-200 flex items-center justify-center font-mono text-[8px]">|||||||||||||||||</div>
                              <div className="text-[8px] font-mono mt-0.5">{label.barcode}</div>
                            </div>
                            <div className="text-[9px] font-medium leading-tight line-clamp-2">{label.productName}</div>
                            <div className="text-[12px] font-bold mt-1 text-emerald-700">Rs.{label.price}</div>
                          </div>
                        ))}
                        {strip.length === 1 && (
                          <div className="flex-1 bg-slate-900/50 p-2 rounded border border-dashed border-slate-800 text-slate-600 flex items-center justify-center italic text-xs">
                            Empty Space (Right)
                          </div>
                        )}
                      </div>
